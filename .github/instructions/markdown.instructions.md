...
- It is **not** currently a required status check for merging into `main` (only `run (3.9/3.10/3.11)` and `e2e-tests` are required), so pre-existing failures won't block your PR.
